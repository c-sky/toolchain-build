...
{
		{ 0, CK_UART_ADDRBASE0,	CK_UART0_IRQID, 0, FALSE, NULL },
};
...
{
  CK_Uart_ChangeBaudrate(uartid,BAUDRATE);
...
CK_INT32 CK_Uart_Open(CK_Uart_Device uartid,void (*handler)(CK_INT8 error))
{
  ...
  irqhander->priority = info->priority;
...
CK_INT32 CK_Uart_PutChar(CK_Uart_Device uartid, CK_UINT8 ch)
{
  ...
  else
  {
    if(ch == '\n')
    {
      while (CK_CircleBuffer_Write(&(info->txcirclebuffer), '\r') == FALSE );
      while (CK_CircleBuffer_Write(&(info->txcirclebuffer), ch) == FALSE );
     }
     else
     {
    	 while (CK_CircleBuffer_Write(&(info->txcirclebuffer), ch) == FALSE );
